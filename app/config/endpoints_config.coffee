    ...
    search_types:
      filter:
        ...
        builder: (opts) ->
          ...
          unless opts.species is '' or opts.behavior is ''
            criteria.push "behaviors @> '{ \"#{ opts.species }\", \"#{ opts.behavior }\" }'"
        ...
        params:
          ...
          behavior:
            type: 'Select'
            options: ['', 'eating', 'interacting', 'moving', 'resting', 'standing']
            validation: (value, collection)->
              param = collection.find (model) -> model.get('key') is 'species'
              species = param.attributes.val
              value is '' or species isnt ''
          site:
            type: 'Select'
